---
title: LSP Servers
description: shob integrates with your LSP servers.
---

shob integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

shob comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `shob_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
You can customize LSP servers through the `lsp` section in your shob config.
