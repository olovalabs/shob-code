...
With shob you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, shob uses our own `shob` theme.
...
shob comes with several built-in themes.
...
- Want shob to match their terminal's appearance
...
shob supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/shob/themes/*.json` or `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.shob/themes/*.json`
4. **Current working directory** - Defined in `./.shob/themes/*.json`
