---
title: Servidor
description: Interaja com o servidor shob via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `shob serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente shob pode usar.
...
```bash
shob serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `shob.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
shob serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `shob_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `shob`, ou defina `shob_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `shob serve` quanto ao `shob web`.

```bash
shob_SERVER_PASSWORD=your-password shob serve
```
...
Quando você executa `shob`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor shob para interagir com o shob programaticamente.
:::

Essa arquitetura permite que o shob suporte múltiplos clientes e permite que você interaja com o shob programaticamente.

Você pode executar `shob serve` para iniciar um servidor autônomo. Se você tiver o TUI do shob em execução, `shob serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do shob [IDE](/docs/ide).
...
O servidor shob expõe as seguintes APIs.
