...
As habilidades do agente permitem que o shob descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O shob pesquisa nesses locais:

- Configuração do projeto: `.shob/skills/<name>/SKILL.md`
- Configuração global: `~/.config/shob/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o shob sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.shob/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.shob/skills/git-release/SKILL.md` assim:
...
O shob lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `shob.json`:
...
**Para agentes embutidos** (em `shob.json`):
