...
Com o shob, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o shob usa nosso próprio tema `shob`.
...
O shob vem com vários temas embutidos.
...
- Querem que o shob corresponda à aparência do seu terminal
...
O shob suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/shob/themes/*.json` ou `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.shob/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.shob/themes/*.json`
