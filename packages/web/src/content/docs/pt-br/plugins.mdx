---
title: Plugins
description: Escreva seus próprios plugins para estender o shob.
---

Plugins permitem que você estenda o shob conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do shob.
...
- `.shob/plugins/` - Plugins em nível de projeto
- `~/.config/shob/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/shob/node_modules/`.
...
1. Configuração global (`~/.config/shob/shob.json`)
2. Configuração do projeto (`shob.json`)
3. Diretório de plugins global (`~/.config/shob/plugins/`)
4. Diretório de plugins do projeto (`.shob/plugins/`)
...
O shob executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do shob para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o shob.
...
:::note
Se você estiver usando o aplicativo desktop shob, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o shob de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao shob:
...
O helper `tool` cria uma ferramenta personalizada que o shob pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o shob junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://shob.ai/docs/sdk) para detalhes.
