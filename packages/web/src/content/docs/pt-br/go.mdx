...
O shob Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O shob Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no shob. Você assina o shob Go e
...
O shob Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O shob Go funciona como qualquer outro provedor no shob.

1. Você entra no **<a href={console}>shob Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `shob Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o shob Go.
:::
...
O shob Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do shob
usa o formato `shob-go/<model-id>`. Por exemplo, para o Kimi K2.5, você usaria
`shob-go/kimi-k2.5` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o shob também.
