...
Existem dois tipos de agentes no shob; agentes primários e subagentes.
...
shob vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
shob vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

shob vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `shob.json`:
...
Se nenhuma temperatura for especificada, o shob usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do shob quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do shob usa o formato `provider/model-id`. Por exemplo, se você estiver usando [shob Zen](/docs/zen), você usaria `shob/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `shob models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/shob).
:::
