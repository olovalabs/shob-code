...
Det er to typer agenter i shob; primære agenter og underagenter.
...
shob kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
shob kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

shob kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `shob.json`:
...
- Globalt: `~/.config/shob/agents/`
- Per prosjekt: `.shob/agents/`
...
Hvis ingen temperatur er spesifisert, bruker shob modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale shob-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i shob-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [shob Zen](/docs/zen), vil du bruke `shob/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `shob models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/shob).
:::
