...
Du kan legge til eksterne verktøy i shob ved å bruke _Model Context Protocol_ eller MCP. shob støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [shob Config](https://shob.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="shob.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/shob`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
shob håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil shob:
...
Hvis serveren krever autentisering, vil shob be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `shob mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil shob lagre tokens sikkert i `~/.local/share/shob/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i shob, sammen med innebygde verktøy. Så du kan administrere dem gjennom shob-konfigurasjonen som alle andre verktøy.
...
```bash
shob mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble shob til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver shob-samtale oppført som en separat økt.
