...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i shob-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i shob lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom shob-konfigurasjonen.
...
shob kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.shob/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `shob.json` konfigurasjonsfil:
...
Hvis ingen temperatur er spesifisert, bruker shob modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale shob-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.shob/modes/` for prosjektspesifikke moduser eller `~/.config/shob/modes/` for globale moduser:
