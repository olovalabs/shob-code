---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i shob.
---

import config from "../../../../config.mjs"
export const console = config.console

shob bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i shob-konfigurasjonen.
...
i `~/.local/share/shob/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i shob
konfig.
...
shob Zen er en liste over modeller levert av shob-teamet som har vært
testet og verifisert for å fungere godt med shob. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med shob Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `shob Zen` og gå til [shob.ai/auth](https://shob.ai/zen).
...
Det fungerer som alle andre leverandører i shob og er helt valgfritt å bruke.

---

## shob Go

shob Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av shob-teamet som har vært
testet og verifisert for å fungere godt med shob.

1. Kjør kommandoen `/connect` i TUI, velg `shob Go`, og gå til [shob.ai/auth](https://shob.ai/zen).
...
Slik bruker du Amazon Bedrock med shob:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører shob:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `shob.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i shob støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at shob skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom shob-konfigurasjonen.
...
for å lagre token i shob auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
shob bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse shob
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`shob.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `shob.json`:
...
Slik bruker du GitHub Copilot-abonnementet med shob:
...
Slik bruker du Google Vertex AI med shob:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom shob, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session) plugin for automatisk å logge hver shob-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver shob-samtale oppført som en separat økt.
...
Du kan konfigurere shob for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere shob for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere shob for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for shob. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/shob) for detaljer.
:::
...
Slik bruker du Ollama Cloud med shob:
...
3. Kopier API-nøkkelen for bruk i shob.
...
6. **Viktig**: Før du bruker skymodeller i shob, må du hente modellinformasjonen lokalt:
...
shob Zen er en liste over testede og verifiserte modeller levert av shob-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>shob Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **shob Zen**.

   ```txt
   /connect
   ```

3. Skriv inn shob API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom shob-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom shob-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med shob:
...
Du kan også tilpasse modeller gjennom shob-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med shob. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `shob.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar shob forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `shob auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk shob-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i shob-konfigurasjonen.
