...
description: Skriv dine egne programtillegg for å utvide shob.
---

Plugins lar deg utvide shob ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til shob.
...
- `.shob/plugins/` - Programtillegg på prosjektnivå
- `~/.config/shob/plugins/` - Globale plugins
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/shob/node_modules/`.
...
1. Global konfigurasjon (`~/.config/shob/shob.json`)
2. Prosjektkonfigurasjon (`shob.json`)
3. Global plugin-katalog (`~/.config/shob/plugins/`)
4. Prosjektpluginkatalog (`.shob/plugins/`)
...
shob kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En shob SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide shob.
...
:::note
Hvis du bruker shob-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre shob fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til shob:
...
`tool`-hjelperen lager et tilpasset verktøy som shob kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for shob sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://shob.ai/docs/sdk) for detaljer.
