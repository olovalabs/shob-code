...
You can add external tools to shob using the _Model Context Protocol_, or MCP. shob supports both local and remote servers.
...
You can define MCP servers in your [shob Config](https://shob.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/shob` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
shob automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, shob will:
...
If the server requires authentication, shob will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `shob mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, shob will store the tokens securely in `~/.local/share/shob/mcp-auth.json`.
...
Your MCPs are available as tools in shob, alongside built-in tools. So you can manage them through the shob config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect shob to your Sentry account.
