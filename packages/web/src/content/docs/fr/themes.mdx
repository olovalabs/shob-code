...
Avec shob, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, shob utilise notre propre thème `shob`.
...
shob est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que shob corresponde à l'apparence de leur terminal
...
shob prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/shob/themes/*.json` ou `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.shob/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.shob/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/shob/themes
vim ~/.config/shob/themes/my-theme.json
```
...
{
  "$schema": "https://shob.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
