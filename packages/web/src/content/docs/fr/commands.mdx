...
Créez `.shob/commands/test.md` :

```md title=".shob/commands/test.md"
---
...
Vous pouvez ajouter des commandes personnalisées via la configuration shob ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre shob [config](/docs/config) :

```json title="shob.jsonc" {4-12}
{
...
- Global : `~/.config/shob/commands/`
- Par projet : `.shob/commands/`

```markdown title="~/.config/shob/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".shob/commands/component.md"
---
...
```md title=".shob/commands/create-file.md"
---
...
```md title=".shob/commands/analyze-coverage.md"
---
...
```md title=".shob/commands/review-changes.md"
---
...
```md title=".shob/commands/review-component.md"
---
...
shob comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
