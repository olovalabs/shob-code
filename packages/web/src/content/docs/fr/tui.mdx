---
title: TUI
description: Utilisation de l’interface utilisateur du terminal shob.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

shob fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de shob démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le shob TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à shob. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `shob.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `shob_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
