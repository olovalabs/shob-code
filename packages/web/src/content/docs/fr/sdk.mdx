---
title: SDK
description: Client JS de type sécurisé pour le serveur shob.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK shob JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler shob par programme.
...
Créez une instance de shob :

```javascript
import { createshob } from "@shob-ai/sdk"

const { client } = await createshob()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `shob.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createshob } from "@shob-ai/sdk"

const shob = await createshob({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${shob.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de shob, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createshobClient } from "@shob-ai/sdk"

const client = createshobClient({
...
```typescript
import type { Session, Message, Part } from "@shob-ai/sdk"
```
