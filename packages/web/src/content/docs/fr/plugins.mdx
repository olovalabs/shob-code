---
title: Plugins
description: Écrivez vos propres plugins pour étendre shob.
---

Les plugins vous permettent d'étendre shob en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de shob.
...
- `.shob/plugins/` - Plugins au niveau du projet
- `~/.config/shob/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/shob/node_modules/`.
...
1. Configuration globale (`~/.config/shob/shob.json`)
2. Configuration du projet (`shob.json`)
3. Répertoire global des plugins (`~/.config/shob/plugins/`)
4. Répertoire des plugins du projet (`.shob/plugins/`)
...
shob exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK shob pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre shob.
...
:::note
Si vous utilisez l'application de bureau shob, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez shob de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à shob :
...
L'assistant `tool` crée un outil personnalisé que shob peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour shob aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://shob.ai/docs/sdk) pour plus de détails.
