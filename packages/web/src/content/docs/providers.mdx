---
title: Providers
description: Using any LLM provider in shob.
---

import config from "../../../config.mjs"
export const console = config.console

shob uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your shob config.
...
in `~/.local/share/shob/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your shob
config.
...
shob Zen is a list of models provided by the shob team that have been
tested and verified to work well with shob. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with shob Zen.
:::

1. Run the `/connect` command in the TUI, select `shob Zen`, and head to [shob.ai/auth](https://shob.ai/zen).
...
It works like any other provider in shob and is completely optional to use.

---

## shob Go

shob Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the shob team that have been
tested and verified to work well with shob.

1. Run the `/connect` command in the TUI, select `shob Go`, and head to [shob.ai/auth](https://shob.ai/zen).
...
To use Amazon Bedrock with shob:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running shob:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `shob.json`:
...
shob. Anthropic explicitly prohibits this.

Previous versions of shob came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in shob with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for shob to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your shob config.
...
GitLab Duo support in shob is experimental. Features, configuration, and
behavior may change in future releases.
:::

shob integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
shob uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock shob
to only use your own GitLab-hosted instance, add the following to your
`shob.json` file. It is also recommended to disable session sharing.
...
More documentation on [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, shob will:
...
4. Route tool execution requests through shob's permission-gated tool system
...
To use your GitHub Copilot subscription with shob:
...
To use Google Vertex AI with shob:
...
In the event you see a feature or model from Helicone that isn't configured automatically through shob, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session) plugin to automatically log each shob conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each shob conversation listed as a separate session.
...
You can configure shob to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure shob to use local models through LM Studio.
...
You can configure shob to use local models through Ollama.

:::tip
Ollama can automatically configure itself for shob. See the [Ollama integration docs](https://docs.ollama.com/integrations/shob) for details.
:::
...
3. Copy the API key for use in shob.
...
6. **Important**: Before using cloud models in shob, you must pull the model information locally:
...
shob Zen is a list of tested and verified models provided by the shob team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>shob Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **shob Zen**.

   ```txt
   /connect
   ```

3. Enter your shob API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your shob config.
...
5. You can also customize them through your shob config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with shob:
...
You can also customize models through your shob config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with shob. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `shob.json` file in your project directory:
...
The `limit` fields allow shob to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `shob auth list` to see if the credentials
...
2. For custom providers, check the shob config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your shob config.
