...
Создайте `.shob/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию shob или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем shob [config](/docs/config):
...
- Глобальный: `~/.config/shob/commands/`
- Для каждого проекта: `.shob/commands/`
...
shob включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
