...
Ajan becerileri, shob'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
shob su konumlari tarar:

- Proje konfigurasyonu: `.shob/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/shob/skills/<name>/SKILL.md`
...
Proje yerel yollarinda shob, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.shob/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.shob/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
shob, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`shob.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`shob.json` icinde):
