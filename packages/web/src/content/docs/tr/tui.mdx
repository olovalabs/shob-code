---
title: TUI
description: shob terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

shob, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

shob'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
shob TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
shob'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
shob'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `shob.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `shob_TUI_CONFIG` kullanın.
