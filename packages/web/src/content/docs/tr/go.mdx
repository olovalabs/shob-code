...
shob Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
shob Go şu anda beta olarak mevcut.
:::

Go, shob'daki diğer sağlayıcılar gibi çalışır. shob Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve shob'u kullanmak için bunu kullanmanıza gerek yoktur.
...
shob Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

shob Go, shob'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>shob Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `shob Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye shob Go'ya abone olabilir.
:::
...
shob Go aşağıdaki limitleri içerir:
...
shob yapılandırmanızdaki [model id](/docs/config/#models) formatı `shob-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `shob-go/kimi-k2.5` kullanmalısınız.
...
shob Go'yu şu amaçlarla oluşturduk:
...
4. shob ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
