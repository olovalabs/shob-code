...
shob ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak shob kendi `shob` temasını kullanır.
...
shob birden fazla yerleşik temayla gelir.
...
- shob'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
shob, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/shob/themes/*.json` veya `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.shob/themes/*.json`
4. **Geçerli çalışma dizini** - `./.shob/themes/*.json`
