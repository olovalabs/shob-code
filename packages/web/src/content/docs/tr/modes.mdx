...
:::caution
Modlar artık shob yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

shob'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya shob yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
shob iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.shob/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`shob.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, shob modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global shob yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.shob/modes/` veya genel modlar için `~/.config/shob/modes/`'de mod dosyaları oluşturun:
