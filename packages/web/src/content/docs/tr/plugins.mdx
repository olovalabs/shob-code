---
title: Eklentiler
description: shob'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek shob'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya shob'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.shob/plugins/` - Proje düzeyinde eklentiler
- `~/.config/shob/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/shob/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/shob/shob.json`)
2. Project config (`shob.json`)
3. Global eklenti dizini (`~/.config/shob/plugins/`)
4. Proje eklenti dizini (`.shob/plugins/`)
...
shob bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir shob SDK istemcisi.
...
shob'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
shob masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
shob'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca shob'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, shob'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra shob'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://shob.ai/docs/sdk) bakın.
