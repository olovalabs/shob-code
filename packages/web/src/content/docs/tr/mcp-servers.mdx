...
_Model Bağlam Protokolü_ veya MCP kullanarak shob'a harici araçlar ekleyebilirsiniz. shob hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [shob Config](https://shob.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/shob` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
shob, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde shob şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa shob, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `shob mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından shob, belirteçleri `~/.local/share/shob/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra shob'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi shob yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve shob'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
