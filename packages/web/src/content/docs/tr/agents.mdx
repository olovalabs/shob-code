...
shob'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
shob, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
shob, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

shob iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `shob.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse shob modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global shob yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
shob yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [shob Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `shob/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `shob models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/shob).
:::
