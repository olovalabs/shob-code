---
title: Sunucu
description: shob sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`shob serve` komutu, shob istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `shob.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `shob_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `shob` değeridir; değiştirmek isterseniz `shob_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `shob serve` hem de `shob web` için geçerlidir.
...
`shob` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
shob ile programatik etkileşim için shob sunucusunu kullanın.
:::

Bu mimari, shob'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`shob serve` ile bağımsız bir sunucu başlatabilirsiniz. shob TUI açıksa, `shob serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum shob [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
shob sunucusu aşağıdaki API'leri sunar.
