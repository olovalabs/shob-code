...
description: shob, LSP sunucularınızla bütünleşir.
---

shob, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

shob, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`shob_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, shob yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
