---
title: Sağlayıcılar
description: shob'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

shob, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı shob yapılandırmanızda yapılandırın.
...
`~/.local/share/shob/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları shob'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
shob Zen, shob ekibi tarafından sağlanan modellerin bir listesidir.
shob ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz shob Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, shob'u seçin ve [shob.ai/auth](https://shob.ai/auth)'ye gidin.
...
shob'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## shob Go

shob Go, shob ile iyi çalıştığı test edilmiş ve doğrulanmış, shob ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `shob Go`'yu seçin ve [shob.ai/auth](https://shob.ai/zen) adresine gidin.
...
Amazon Bedrock'u shob ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   shob'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `shob.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi shob'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   shob'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   shob yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in shob auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
shob, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. shob'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`shob.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth) ana sayfasında.

##### Yapılandırma

`shob.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi shob'la kullanmak için:
...
Google Vertex AI'yi shob ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları shob çalıştırılırken ayarlayın.
...
Helicone'un shob aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her shob görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her shob görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
shob'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla shob'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla shob'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini shob için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/shob)'a bakın.
:::
...
Ollama Cloud'u shob ile kullanmak için:
...
3. shob'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: shob'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
shob Zen, shob ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>shob Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **shob Zen**'i arayın.

   ```txt
   /connect
   ```

3. shob API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   shob yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca shob yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi shob ile kullanmak için:
...
Modelleri shob yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı shob ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `shob.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları shob'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `shob auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için shob yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, shob yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
