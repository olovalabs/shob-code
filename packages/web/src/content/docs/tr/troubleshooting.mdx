...
shob ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\shob\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `shob --log-level DEBUG`.

---

## Depolama

shob, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/shob/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\shob` yapistirin
...
shob Desktop arka planda yerel bir shob sunucusu (`shob-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `shob` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/shob/shob.jsonc` (veya `~/.config/shob/shob.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/shob/shob.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\shob\shob.jsonc` yapistirin
...
shob diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/shob/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\shob\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), shob'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. shob Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/shob`
- **Linux**: `~/.cache/shob` dizinini silin (veya `rm -rf ~/.cache/shob` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\shob` yapistirin

3. shob Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

shob Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`shob.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `shob_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `shob_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta shob Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, shob ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

shob Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda shob icin bildirimler etkinse
...
1. shob Desktop'u kapatin
2. Su dosyalari bulun ve silin (shob Desktop uygulama veri dizininde yer alirlar):

- `shob.settings.dat` (desktop varsayilan sunucu URL'si)
- `shob.global.dat` ve `shob.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
shob ile ilgili bir sorun yasiyorsaniz:
...
3. `shob upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `shob models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\shob`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. shob, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\shob`

2. En guncel provider paketlerini yeniden kurmak icin shob'u yeniden baslatin

Bu, shob'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
shob, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
