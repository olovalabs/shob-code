...
Para depurar problemas con shob, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/shob/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\shob\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `shob --log-level DEBUG`.

---

## Almacenamiento

shob almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/shob/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\shob`
...
shob Desktop ejecuta un servidor shob local (el sidecar `shob-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `shob` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/shob/shob.jsonc` (o `~/.config/shob/shob.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/shob/shob.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\shob\shob.jsonc`
...
shob también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/shob/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\shob\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.shob/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que shob pueda reconstruirlo.

1. Salga de shob Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/shob`
- **Linux**: eliminar `~/.cache/shob` (o ejecutar `rm -rf ~/.cache/shob`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\shob`

3. Reinicie el escritorio shob.

---

### Solucionar problemas de conexión del servidor

shob Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `shob.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `shob_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `shob_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio shob requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de shob.

---

### Notificaciones que no se muestran

shob Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para shob en la configuración de su sistema operativo, y
...
1. Salga del escritorio shob.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio shob):

- `shob.settings.dat` (URL del servidor predeterminado de escritorio)
- `shob.global.dat` y `shob.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/shob/issues**](https://x.com/devshowcse)
...
2. **Únete a nuestro Discord**
   ...
   [**shob.ai/discord**](https://shob.ai/discord)
...
3. Asegúrese de tener la última versión con `shob upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `shob models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/shob
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\shob`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. shob instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\shob`

2. Reinicie shob para reinstalar los paquetes del proveedor más recientes.

Esto obligará a shob a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
shob detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
