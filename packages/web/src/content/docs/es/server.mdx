---
title: Servidor
description: Interactuar con el servidor shob a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `shob serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente shob puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `shob.local` |
...
Configure `shob_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `shob`, o configure `shob_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `shob serve` como a `shob web`.
...
Cuando ejecuta `shob`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor shob para interactuar con shob mediante programación.
:::

Esta arquitectura permite que shob admita múltiples clientes y le permite interactuar con shob mediante programación.

Puede ejecutar `shob serve` para iniciar un servidor independiente. Si tienes el
shob TUI ejecutándose, `shob serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos shob [IDE](/docs/ide).
...
El servidor shob expone las siguientes API.
