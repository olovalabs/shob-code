...
shob Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
shob Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en shob. Te suscribes a shob Go y
...
shob Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

shob Go funciona como cualquier otro proveedor en shob.

1. Inicias sesión en **<a href={console}>shob Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `shob Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a shob Go.
:::
...
shob Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de shob
usa el formato `shob-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías
`shob-go/kimi-k2.5` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con shob.
