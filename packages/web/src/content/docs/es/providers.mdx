---
title: Proveedores
description: Usando cualquier proveedor LLM en shob.
---

import config from "../../../../config.mjs"
export const console = config.console

shob usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración shob.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu shob
configuración.
...
shob Zen es una lista de modelos proporcionados por el equipo shob que han sido
probado y verificado para funcionar bien con shob. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con shob Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione shob y diríjase a [shob.ai/auth](https://shob.ai/auth).
...
Funciona como cualquier otro proveedor en shob y su uso es completamente opcional.

---

## shob Go

shob Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de shob que han sido
probado y verificado para funcionar bien con shob.

1. Ejecute el comando `/connect` en TUI, seleccione `shob Go` y diríjase a [shob.ai/auth](https://shob.ai/zen).
...
Para usar Amazon Bedrock con shob:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta shob:
   ...
   Para una configuración persistente o específica del proyecto, utilice `shob.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en shob no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que shob funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración shob.
...
para almacenar el token en el almacenamiento de autenticación shob.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
shob utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear shob
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `shob.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth).

##### Configuración

Personalizar a través de `shob.json`:
...
Para utilizar su suscripción GitHub Copilot con shob:
...
Para utilizar Google Vertex AI con shob:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta shob.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de shob, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session) para registrar automáticamente cada conversación shob como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación shob enumerada como una sesión separada.
...
Puede configurar shob para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar shob para usar modelos locales a través de LM Studio.
...
Puedes configurar shob para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para shob. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/shob) para obtener más detalles.
:::
...
Para usar Ollama Cloud con shob:
...
3. Copie la clave API para usarla en shob.
...
6. **Importante**: Antes de usar modelos en la nube en shob, debe obtener la información del modelo localmente:
...
shob Zen es una lista de modelos probados y verificados proporcionada por el equipo shob. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>shob Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **shob Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave shob API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración shob.
...
5. También puedes personalizarlos a través de tu configuración shob. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con shob:
...
También puede personalizar modelos a través de su configuración shob. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con shob. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `shob.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a shob comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `shob auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración shob y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración shob.
