---
title: Complementos
description: Escriba sus propios complementos para extender shob.
---

Los complementos le permiten extender shob al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de shob.
...
- `.shob/plugins/` - Complementos a nivel de proyecto
- `~/.config/shob/plugins/` - Complementos globales
...
```json title="shob.json"
{
  ...
  "plugin": ["shob-helicone-session", "shob-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/shob/node_modules/`.
...
1. Configuración global (`~/.config/shob/shob.json`)
2. Configuración del proyecto (`shob.json`)
3. Directorio global de complementos (`~/.config/shob/plugins/`)
4. Directorio de complementos del proyecto (`.shob/plugins/`)
...
shob ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".shob/plugins/my-plugin.ts"
...
```js title=".shob/plugins/example.js"
...
- `client`: Un cliente SDK shob para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar shob.
...
```js title=".shob/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "shob"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio shob, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que shob lea archivos `.env`:

```javascript title=".shob/plugins/env-protection.js"
...
```javascript title=".shob/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a shob:

```ts title=".shob/plugins/custom-tools.ts"
import { type Plugin, tool } from "@shob-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que shob puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para shob junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://shob.ai/docs/sdk) para obtener más detalles.
...
```ts title=".shob/plugins/compaction.ts"
import type { Plugin } from "@shob-ai/plugin"
...
```ts title=".shob/plugins/custom-compaction.ts"
import type { Plugin } from "@shob-ai/plugin"
