...
Las herramientas permiten que LLM realice acciones en su código base. shob viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en shob.
...
:::note
Esta herramienta solo está disponible cuando `shob_EXPERIMENTAL_LSP_TOOL=true` (o `shob_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor shob o cuando la variable de entorno `shob_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar shob:

```bash
shob_ENABLE_EXA=1 shob
```
