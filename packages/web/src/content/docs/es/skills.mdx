...
Las habilidades del agente permiten a shob descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
shob busca estas ubicaciones:

- Configuración del proyecto: `.shob/skills/<name>/SKILL.md`
- Configuración global: `~/.config/shob/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, shob sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.shob/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.shob/skills/git-release/SKILL.md` así:
...
shob enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `shob.json`:
...
**Para agentes integrados** (en `shob.json`):
