...
description: Cree herramientas que LLM pueda llamar en shob.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de shob como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.shob/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/shob/tools/`.
...
```ts title=".shob/tools/database.ts" {1}
...
```ts title=".shob/tools/project.ts" {8}
import { tool } from "@shob-ai/plugin"
...
```python title=".shob/tools/add.py"
import sys
...
```ts title=".shob/tools/python-add.ts" {10}
import { tool } from "@shob-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".shob/tools/add.py")
