...
Con shob puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, shob usa nuestro propio tema `shob`.
...
shob viene con varios temas integrados.
...
- Quiere que shob coincida con la apariencia de su terminal
...
shob admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/shob/themes/*.json` o `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.shob/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.shob/themes/*.json`
