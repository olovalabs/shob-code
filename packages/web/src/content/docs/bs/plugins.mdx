---
title: Dodaci
description: Napišite vlastite dodatke za proširenje shob.
---

Dodaci vam omogućavaju da proširite shob spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja shob.
...
- `.shob/plugins/` - Dodaci na nivou projekta
- `~/.config/shob/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/shob/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/shob/shob.json`)
2. Konfiguracija projekta (`shob.json`)
3. Globalni direktorij dodataka (`~/.config/shob/plugins/`)
4. Direktorij dodataka projekta (`.shob/plugins/`)
...
shob pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: shob SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje shob.
...
Spriječite shob da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u shob:
...
Pomoćnik `tool` kreira prilagođeni alat koji shob može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za shob zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://shob.ai/docs/sdk) za detalje.
