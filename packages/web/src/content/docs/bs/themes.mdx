...
U shob mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, shob koristi nasu `shob` temu.
...
shob dolazi sa vise ugradenih tema.
...
- Zele da shob odgovara izgledu njihovog terminala
...
shob podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/shob/themes/*.json` ili `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.shob/themes/*.json`
4. **Trenutni radni direktorij** - `./.shob/themes/*.json`
