...
Agent skills omogucavaju shob da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
shob pretrazuje ove lokacije:

- Konfiguracija projekta: `.shob/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/shob/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, shob ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.shob/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.shob/skills/git-release/SKILL.md` ovako:
...
shob navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `shob.json`:
...
**Za ugradene agente** (u `shob.json`):
