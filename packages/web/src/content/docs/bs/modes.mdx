...
ove ili konfigurirajte svoje putem shob config.
...
shob dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.shob/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `shob.json` konfiguracijskom fajlu:
...
Ako temperatura nije navedena, shob koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.shob/modes/` za specifične načine rada ili `~/.config/shob/modes/` za globalne načine:
