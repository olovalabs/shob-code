---
title: Server
description: Komunicirajte s shob serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `shob serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji shob klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `shob.local` |
...
Postavite `shob_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `shob`, ili postavite `shob_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `shob serve` i za `shob web`.
...
Kada pokrenete `shob`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite shob server za programsku interakciju sa shob-om.
:::

Ova arhitektura omogucava shob podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `shob serve` da startate standalone server. Ako je shob TUI vec pokrenut, `shob serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste shob [IDE](/docs/ide) pluginovi.
...
shob server izlaže sljedece API-je.
