---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u shob.
---

import config from "../../../../config.mjs"
export const console = config.console

shob koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj shob konfiguraciji.
...
u `~/.local/share/shob/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem shob
config.
...
shob Zen je lista modela koje je obezbedio shob tim koji su bili
testirano i potvrđeno da dobro radi sa shob. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa shob Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite shob i idite na [shob.ai/auth](https://shob.ai/auth).
...
Radi kao i svaki drugi provajder u shob i potpuno je opcionalan za korištenje.

---

## shob Go

shob Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža shob tim i koji su testirani i verificirani da dobro rade s shob-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `shob Go` i idite na [shob.ai/auth](https://shob.ai/zen).
...
Da biste koristili Amazon Bedrock s shob:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete shob:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `shob.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u shob.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi shob ispravno radio.
   :::
...
Također možete dodati modele kroz svoju shob konfiguraciju.
...
da pohrani token u shob auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
shob koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate shob
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`shob.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth).

##### Konfiguracija

Prilagodite putem `shob.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s shob:
...
Za korištenje Google Vertex AI s shob:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće shob.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem shoba, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session) da automatski prijavite svaki shob razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki shob razgovor naveden kao zasebna sesija.
...
Možete konfigurirati shob za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati shob za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati shob za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za shob. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/shob) za detalje.
:::
...
Da biste koristili Ollama Cloud s shob:
...
3. Kopirajte API ključ za korištenje u shob.
...
6. **Važno**: Prije upotrebe modela oblaka u shob, morate lokalno povući informacije o modelu:
...
shob Zen je lista testiranih i verifikovanih modela koju je obezbedio shob tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>shob Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **shob Zen**.

```txt
   /connect
```

3. Unesite svoj shob API ključ.
...
Također možete dodati dodatne modele putem vaše shob konfiguracije.
...
5. Također ih možete prilagoditi putem vaše shob konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa shobom:
...
Također možete prilagoditi modele kroz svoju shob konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s shob-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `shob.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju shob da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `shob auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite shob konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj shob konfiguraciji.
