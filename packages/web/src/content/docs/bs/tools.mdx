...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. shob dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u shob.
...
:::note
Ovaj alat je dostupan samo kada je `shob_EXPERIMENTAL_LSP_TOOL=true` (ili `shob_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz shob provajdera ili kada je varijabla `shob_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju shob:
