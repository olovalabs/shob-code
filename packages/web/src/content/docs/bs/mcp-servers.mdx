...
Možete dodati vanjske alate u shob koristeći _Model Context Protocol_, ili MCP. shob podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [shob Config](https://shob.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/shob`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
shob automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, shob će:
...
Ako server zahtijeva autentifikaciju, shob će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `shob mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, shob sigurno čuva tokene u `~/.local/share/shob/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u shob, zajedno s ugrađenim alatima. Možete njima upravljati kroz shob konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete shob sa vašim Sentry nalogom.
