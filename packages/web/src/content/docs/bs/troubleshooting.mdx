...
Da biste otklonili probleme s shob, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\shob\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `shob --log-level DEBUG`.

---

## Pohrana

shob pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/shob/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\shob`
...
shob Desktop pokreće lokalni shob server (`shob-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `shob` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/shob/shob.jsonc` (ili `~/.config/shob/shob.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/shob/shob.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\shob\shob.jsonc`
...
shob također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/shob/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\shob\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga shob mogao ponovo izgraditi.

1. Potpuno zatvorite shob Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/shob`
- **Linux**: obrišite `~/.cache/shob` (ili pokrenite `rm -rf ~/.cache/shob`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\shob`

3. Ponovo pokrenite shob Desktop.

---

### Rješavanje problema sa vezom na serveru

shob Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `shob.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `shob_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `shob_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, shob Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa shob karakteristikama.

---

### Obavještenja se ne prikazuju

shob Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za shob u postavkama vašeg OS-a, i
...
1. Zatvorite shob Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka shob Desktop aplikacije):

- `shob.settings.dat` (URL zadanog servera za desktop)
- `shob.global.dat` i `shob.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `shob upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `shob models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\shob`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. shob dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\shob`

2. Ponovo pokrenite shob da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti shob da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
shob će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
