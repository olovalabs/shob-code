...
shob Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
shob Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w shob. Subskrybujesz shob Go i
...
shob Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

shob Go działa jak każdy inny dostawca w shob.

1. Logujesz się do **<a href={console}>shob Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `shob Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować shob Go.
:::
...
shob Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji shob
używa formatu `shob-go/<model-id>`. Na przykład dla Kimi K2.5 należy użyć
`shob-go/kimi-k2.5` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z shob.
