---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć shob.
---

Wtyczki umożliwiające wyłączenie shob poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania shob.
...
- `.shob/plugins/` - Wtyczki na poziomie projektu
- `~/.config/shob/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/shob/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/shob/shob.json`)
2. Konfiguracja projektu (`shob.json`)
3. Globalny katalog wtyczek (`~/.config/shob/plugins/`)
4. Katalog wtyczek projektu (`.shob/plugins/`)
...
shob uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z shob do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do shob.
...
:::note
Jeśli korzystasz z aplikacji komputerowej shob, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj shob czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do shob:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod shob. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://shob.ai/docs/sdk).
