...
Umiejętności agenta pozwalają shob odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
shob przeszukuje te lokalizacje:

- Project config: `.shob/skills/<name>/SKILL.md`
- Global config: `~/.config/shob/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu shob przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.shob/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.shob/skills/git-release/SKILL.md` w ten sposób:
...
shob wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `shob.json`:
...
**Dla agentów wbudowanych** (w `shob.json`):
