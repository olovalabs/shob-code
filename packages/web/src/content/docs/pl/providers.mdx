---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w shob.
---

import config from "../../../../config.mjs"
export const console = config.console

shob używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji shob.
...
w `~/.local/share/shob/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim shob
config.
...
shob Zen to lista modeli dostarczonych przez zespół shob, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z shob. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od shob Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `shob Zen` i przejdź do [shob.ai/auth](https://shob.ai/zen).
...
Działa jak każdy inny dostawca w shob i jest całkowicie opcjonalny w użyciu.

---

## shob Go

shob Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół shob, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z shob.

1. Uruchom polecenie `/connect` w TUI, wybierz `shob Go` i przejdź do [shob.ai/auth](https://shob.ai/zen).
...
Aby używać Amazon Bedrock z shob:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania shob:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `shob.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w shob nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod shob działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji shob.
...
shob używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować shob
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`shob.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [shob-gitlab-auth](https://www.npmjs.com/package/shob-gitlab-auth).

##### Konfiguracja

Customize through `shob.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem shob:
...
Aby używać Google Vertex AI z shob:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania shob.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą shob, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [shob-helicone-session](https://github.com/H2Shami/shob-helicone-session), aby automatycznie rejestrować każdą konwersację shob jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja shob będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować shob tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować shob tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować shob tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem shob. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/shob).
:::
...
Aby korzystać z Ollama Cloud z shob:
...
3. Skopiuj klucz API do użycia w shob.
...
6. **Ważne**: Przed użyciem modeli chmurowych w shob musisz pobrać informacje o modelu lokalnie:
...
shob Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół shob. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>shob Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **shob Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API shob.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji shob.
...
5. Możesz je także dostosować za pomocą konfiguracji shob. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z shob:
...
Możesz także dostosować modele za pomocą konfiguracji shob. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z shob. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `shob.json` w katalogu projektu:
...
Pola `limit` pozwalają shob zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `shob auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację shob i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji shob.
