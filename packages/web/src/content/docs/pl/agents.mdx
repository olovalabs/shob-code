...
W shob obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
shob zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
shob ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

shob ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `shob.json`:
...
- Globalnie: `~/.config/shob/agents/`
- W projekcie: `.shob/agents/`
...
Jeśli nie zostanie skonfigurowana, shob używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji shob, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji shob w `provider/model-id`. Na przykład, [shob Zen](/docs/zen), użyj `shob/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `shob models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/shob).
:::
