...
Możesz dodać dodatkowe informacje do shob za pomocą _Model Context Protocol_, w skrócie MCP. shob obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [shob Config](https://shob.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/shob`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
shob automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, shob:
...
Jeśli serwer wymaga uwierzytelnienia, shob poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `shob mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji shob bezpieczne przechowa tokeny w `~/.local/share/shob/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w shob, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie shob, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć shob z kontem Sentry.
