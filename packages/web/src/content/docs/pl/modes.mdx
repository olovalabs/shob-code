...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji shob.
...
te lub skonfiguruj własne za pomocą konfiguracji shob.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.shob/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `shob.json`:
...
Jeśli nie ma wpływu na temperaturę, shob stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna shob, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.shob/modes/` dla trybów zapisanych dla projektu lub `~/.config/shob/modes/` dla trybów globalnych:
