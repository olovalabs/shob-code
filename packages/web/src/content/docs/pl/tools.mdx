...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. shob posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w shob.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `shob_EXPERIMENTAL_LSP_TOOL=true` (lub `shob_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy shob lub gdy zmienna środowiskowa `shob_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu shob:
