...
Dzięki shob możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie shob używa naszego własnego motywu `shob`.
...
shob ma kilka wbudowanych motywów.
...
- Chcą, aby shob pasował do wyglądu ich terminala
...
shob obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/shob/themes/*.json` lub `$XDG_CONFIG_HOME/shob/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.shob/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.shob/themes/*.json`
