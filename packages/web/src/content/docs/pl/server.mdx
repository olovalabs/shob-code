---
title: Serwer
description: Interakcja z serwerem shob za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `shob serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient shob.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `shob.local` |
...
Ustaw `shob_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `shob` lub ustaw `shob_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `shob serve`, jak i `shob web`.
...
Kiedy uruchomisz `shob`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera shob do programowej interakcji z shob.
:::

Ta architektura umożliwia obsługę wielu klientów przez shob i programową interakcję z kodem otwartym.

Możesz uruchomić `shob serve`, aby uruchomić samodzielny serwer. Jeśli masz
shob TUI działa, `shob serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki shob [IDE](/docs/ide).
...
Serwer shob udostępnia następujące interfejsy API.
