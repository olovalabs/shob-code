...
:::caution
Modes are now configured through the `agent` option in the shob config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in shob allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the shob config.
...
shob comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.shob/plans/*.md` to detail the plan itself
...
Configure modes in your `shob.json` config file:
...
If no temperature is specified, shob uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global shob config and the project specific config.
...
Create mode files in `.shob/modes/` for project-specific modes or `~/.config/shob/modes/` for global modes:
