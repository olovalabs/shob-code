---
title: TUI
description: Using the shob terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

shob provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running shob starts the TUI for the current directory.
...
When using the shob TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to shob. Allows you to select from available providers and add their API keys.
...
This is separate from `shob.json`, which configures server/runtime behavior.
...
Use `shob_TUI_CONFIG` to load a custom TUI config path.
