...
shob has a list of keybinds that you can customize through `tui.json`.
...
shob uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The shob desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `shob.json`.
