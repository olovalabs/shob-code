---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di shob.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

shob fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire shob avvia la TUI per la directory corrente.
...
Quando usi la TUI di shob, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a shob. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `shob.json`, che configura il comportamento di server/runtime.
...
Usa `shob_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
