...
shob Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
shob Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in shob. Ti abboni a shob Go e
...
shob Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

shob Go funziona come qualsiasi altro provider in shob.

1. Accedi a **<a href={console}>shob Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `shob Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a shob Go.
:::
...
shob Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua shob config
utilizza il formato `shob-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti
`shob-go/kimi-k2.5` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con shob.
