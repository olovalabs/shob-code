...
In shob esistono due tipi di agenti: agenti primari e subagenti.
...
shob include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
shob include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

shob include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `shob.json`:
...
- Globale: `~/.config/shob/agents/`
- Per progetto: `.shob/agents/`
...
Se non specifichi una temperature, shob usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di shob sia per la config specifica del progetto.
...
L'ID modello nella configurazione shob usa il formato `provider/model-id`. Per esempio, se stai usando [shob Zen](/docs/zen), useresti `shob/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `shob models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/shob).
:::
