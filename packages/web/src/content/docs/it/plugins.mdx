---
title: Plugin
description: Scrivi plugin per estendere shob.
---

I plugin ti permettono di estendere shob agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di shob.
...
- `.shob/plugins/` - plugin a livello progetto
- `~/.config/shob/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/shob/node_modules/`.
...
1. Config globale (`~/.config/shob/shob.json`)
2. Config di progetto (`shob.json`)
3. Directory plugin globale (`~/.config/shob/plugins/`)
4. Directory plugin di progetto (`.shob/plugins/`)
...
shob esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di shob per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere shob.
...
:::note
Se usi l'app desktop di shob, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a shob di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a shob:
...
L'helper `tool` crea uno strumento personalizzato che shob può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in shob insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://shob.ai/docs/sdk) per i dettagli.
