---
title: Server
description: Interagisci con il server shob via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `shob serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client shob.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `shob.local` |
...
Imposta `shob_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `shob`, oppure imposta `shob_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `shob serve` sia per `shob web`.
...
Quando esegui `shob` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server shob per interagire con shob in modo programmatico.
:::

Questa architettura permette a shob di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `shob serve` per avviare un server standalone. Se la TUI di shob e gia in esecuzione, `shob serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di shob.
...
Il server shob espone le seguenti API.
