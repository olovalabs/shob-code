...
Le skill degli agenti permettono a shob di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
shob cerca in queste posizioni:

- Config di progetto: `.shob/skills/<name>/SKILL.md`
- Config globale: `~/.config/shob/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, shob risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.shob/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/shob/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.shob/skills/git-release/SKILL.md` cosi':
...
shob elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `shob.json`:
...
**Per agenti integrati** (in `shob.json`):
