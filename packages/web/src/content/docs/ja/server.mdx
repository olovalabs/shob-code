---
title: サーバー
description: HTTP 経由で shob サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`shob serve` コマンドは、shob クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `shob.local` |
...
HTTP 基本認証でサーバーを保護するには、`shob_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `shob` になるか、`shob_SERVER_USERNAME` を設定してオーバーライドします。これは、`shob serve` と `shob web` の両方に当てはまります。
...
`shob` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
shob サーバーを使用して、プログラムで shob と対話します。
:::

`shob serve` を実行してスタンドアロンサーバーを起動できます。すでに shob TUI を実行している場合でも、`shob serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、shob [IDE](/docs/ide) プラグインによって使用されます。
