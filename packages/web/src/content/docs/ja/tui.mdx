---
title: TUI
description: shob ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

shob は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

shob を実行すると、現在のディレクトリの TUI が開始されます。
...
shob TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
shob にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
これは、サーバー/ランタイムの動作を構成する `shob.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`shob_TUI_CONFIG` を使用します。
