---
title: Server
description: Interact with shob server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `shob serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an shob client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `shob.local` |
...
Set `shob_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `shob`, or set `shob_SERVER_USERNAME` to override it. This applies to both `shob serve` and `shob web`.
...
When you run `shob` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the shob server to interact with shob programmatically.
:::

This architecture lets shob support multiple clients and allows you to interact with shob programmatically.

You can run `shob serve` to start a standalone server. If you have the
shob TUI running, `shob serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the shob [IDE](/docs/ide) plugins.
...
The shob server exposes the following APIs.
