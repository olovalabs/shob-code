...
import { createshob } from "@shob-ai/sdk"
...
`)
  ...
  const shob = await createshob({ port: 0 })
  ...
  try {
    ...
    const session = await shob.client.session.create()
    const result = await shob.client.session
      .prompt({
